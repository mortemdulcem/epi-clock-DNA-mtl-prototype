...
Blue-Black Color Scheme - Fixed overlapping lines and width
"""
...
# Data - positions with CLEAR SEPARATION - no overlapping
regions_data = {
    'Prefrontal Cortex': {'eaa': 5.3, 'ci': (4.2, 6.5), 'n': 48, 'color': BLUES[0], 
                          'pos': (0.18, 0.32), 'label_pos': (-0.18, 0.32), 'num': 1},
    'Nucleus Accumbens': {'eaa': 4.1, 'ci': (3.2, 5.1), 'n': 36, 'color': BLUES[1],
                          'pos': (0.32, 0.48), 'label_pos': (-0.18, 0.58), 'num': 2},
    'Hippocampus': {'eaa': 3.2, 'ci': (2.3, 4.2), 'n': 24, 'color': BLUES[2],
                    'pos': (0.65, 0.52), 'label_pos': (1.12, 0.52), 'num': 3},
    'Amygdala': {'eaa': 3.5, 'ci': (2.6, 4.4), 'n': 21, 'color': BLUES[3],
                 'pos': (0.48, 0.58), 'label_pos': (1.12, 0.78), 'num': 4},
    'VTA': {'eaa': 2.8, 'ci': (1.9, 3.7), 'n': 18, 'color': BLUES[4],
            'pos': (0.45, 0.72), 'label_pos': (-0.18, 0.84), 'num': 5},
}
...
# Region markers - numbered circles only
for region, data in regions_data.items():
    x = data['pos'][0] * img_w
    y = (1 - data['pos'][1]) * img_h
    
    # Glow effect
    for r, alpha in [(30, 0.12), (22, 0.25)]:
        circle = Circle((x, y), r, facecolor=data['color'], alpha=alpha, zorder=5)
        ax1.add_patch(circle)
    
    # Inner marker with number
    circle = Circle((x, y), 16, facecolor=data['color'], edgecolor='white', 
                    linewidth=2.5, alpha=0.95, zorder=10)
    ax1.add_patch(circle)
    
    ax1.text(x, y, str(data['num']), ha='center', va='center', 
            fontsize=10, fontweight='bold', color='white', zorder=11)

# Labels with connection lines - NO OVERLAP
for region, data in regions_data.items():
    x = data['pos'][0] * img_w
    y = (1 - data['pos'][1]) * img_h
    ...
    # Connection line
    ax1.plot([x, lx], [y, ly], color=data['color'], linewidth=2, alpha=0.7, zorder=8)
...
# Extended limits to show labels
ax1.set_xlim(-img_w * 0.30, img_w * 1.25)
ax1.set_ylim(img_h * 1.02, -img_h * 0.02)
ax1.axis('off')
